...
#from cobra.util.solver import solver_dict
...
try:
	...
	soplex = None
	warn("soplex import failed with error \'{:s}\'".format(str(e)))

#from coralme.solve.symbolic import *
...
def binary_search(me_model, min_mu=0, max_mu=2, mu_accuracy=1e-9,
	...
	:param dict compiled_expressions: precompiled symbolic expressions in the model
	"""
	...
	if compiled_expressions is None:
		compiled_expressions = coralme.solve.compile_expressions(me_model)
	feasible_mu = []
	infeasible_mu = []
	...
	if verbose:
		...
		if debug:
			print("{:s}\tstatus\treset\ttime\titer\tobj".format(mu_str))
		else:
			print("{:s}\tstatus".format(mu_str))
	...
	def try_mu(mu):
		...
		if debug:
			lp.write(filename_base.format(mu), rational=True)
		...
		if debug:
			...
			debug_str = "\t{:s}\t{:.2f}\t{:d}\t{:s}".format(reset_basis, lp.solveTime, lp.numIterations, obj)
		else:
			debug_str = ""
		if status == "optimal":
			if verbose:
				print(success_str_base.format(mu + debug_str))
			...
			if verbose:
				if status != "infeasible" and debug:
					debug_str += "(status: {:s})".format(status)
				print(failure_str_base.format(mu + debug_str))
			return False
	...
	if verbose:
		print("completed in {:.1f} seconds and {:d} iterations".format(time() - start, len(feasible_mu) + len(infeasible_mu)))
	return me_model.solution

def create_lp_at_growth_rate(me_model, growth_rate, compiled_expressions=None, solver=None, **solver_args):
...
def solve_at_growth_rate(me_model, growth_rate, **solver_args):
	lp, solver = create_lp_at_growth_rate(me_model, growth_rate, **solver_args)
	# solve and return
	solver.solve_problem(lp)
	me_model.solution = solver.format_solution(lp, me_model)
