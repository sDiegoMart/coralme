...
class Organism(object):
	...
	def rna_components(self):
		RNA_df = self.RNA_df
		gene_dictionary = self.gene_dictionary['Accession-1'].to_dict()
		return set(gene_dictionary[i] for i in RNA_df['Gene'].values if i in gene_dictionary)
...
#					 "Gene": gene_id,
	...
	def sync_files(self):
		...
		for gene_name,row in gene_dictionary.iterrows():
			...
			if not product_type:
				if 'RNA' in gene_id or 'RNA' in product or product in RNA_df.index:
					product_type = 'RNA'
				elif 'MONOMER' in gene_id or product in complexes_df.index:
