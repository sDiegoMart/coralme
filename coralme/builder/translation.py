...
def add_charged_trna_subreactions(me_model, translation_stop_dict, special_trna_subreactions = {}):
	"""
	...
	for codon in me_model.global_info['stop_codons']:
		...
		subreaction_data = coralme.core.processdata.SubreactionData(stop_codon + '_' + stop_enzyme + '_mediated_termination', me_model)
	...
	for codon, aa in {k:v for k,v in me_model.global_info['codon_table'].forward_table.items() if 'U' not in k}.items():
		...
		subreaction_data = coralme.core.processdata.SubreactionData(amino_acid + '_addition_at_' + codon.replace('T', 'U'), me_model)
