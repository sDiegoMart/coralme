	...
	def generate_files(self):
		...
		if config.get('ref_genbank', None) is not None:
			...
			if bool(config['run_bbh_blast']):
				...
				folder = 'blast_files_and_results'
				...
				# bidirectional blast
				execute('blastp -db {:s}/org -query ref.faa -num_threads 4 -out {:s}/org_as_db.txt -outfmt 6'.format(folder, folder))
				execute('blastp -db {:s}/ref -query org.faa -num_threads 4 -out {:s}/ref_as_db.txt -outfmt 6'.format(folder, folder))
		...
		self.org.biomass_constituents = config['flux_of_biomass_constituents']
...
#			 rule_list = expand_gpr(listify_gpr(rule)).split(" or ")
			...
			if len(rule_list) <= gpr_combination_cutoff:
				...
				n,rule_dict = generify_gpr(listify_gpr(rule),rxn.id,d={})
...
class MEReconstruction(object):
	"""
	MEReconstruction class for reconstructing a ME-Model
	...
	def input_data(self):
		...
		def read(filename):
			return coralme.builder.flat_files.read(filename)

		# Transcriptional Units (Optional)
		filename = config['df_TranscriptionalUnits']
		if pathlib.Path(filename).is_file():
			self.df_tus = read(filename).set_index('TU_id')
		else:
			self.df_tus = pandas.DataFrame(
				columns = ['TU_id', 'replicon', 'genes',
						'start', 'stop', 'tss', 'strand',
						'rho_dependent', 'dnapol'])
			self.df_tus.set_index('TU_id', inplace = True)

		# REQUIRED INPUTS!
		# Reaction Matrix: reactions, metabolites, compartments, stoichiometric coefficientes
		filename = config['df_matrix_stoichiometry']
		self.df_rmsc = read(filename)
		# SubReaction Matrix: subreactions, metabolites, compartments, stoichiometric coefficientes
		filename = config['df_matrix_subrxn_stoich']
		self.df_subs = read(filename)
		# Orphan and Spontaneous reaction metadata
		filename = config['df_metadata_orphan_rxns']
		self.df_rxns = read(filename).set_index('name')
		# Metabolites metadata
		filename = config['df_metadata_metabolites']
		self.df_mets = read(filename).set_index('id')
	...
	def build_me_model(self):
		...
		if len(me.global_info['braun\'s_lipoprotein']) >= 1:
			for Braun_lipoprotein in me.global_info['braun\'s_lipoprotein']:
				...
				murein5px4p = me.metabolites.get_by_id('murein5px4p_p')
